...
function Stat({ label, value }) {
  return (
    <div style={{ textAlign: 'right' }}>
      ...
      <div style={{ fontSize: '1rem', fontWeight: 'bold', color: 'var(--text-primary)', fontFamily: 'monospace' }}>{value}</div>
    </div>
  );
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', width: '100%' }}>
      ...
      <div style={{ background: 'var(--surface-color)', padding: '1rem', borderRadius: '6px', border: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        ...
        <select
          ...
          style={{ background: 'var(--bg-color)', color: 'var(--text-primary)', border: '1px solid var(--border-color)', borderRadius: '4px', padding: '0.5rem 0.75rem', fontFamily: 'monospace', fontSize: '0.95rem', minWidth: '260px' }}
        >
          {trains.map(t => (
                ...
                <tbody>
                  {profile.events.map((e, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid var(--border-color)' }}>
                      <td style={{ padding: '0.5rem', fontFamily: 'monospace' }}>{formatTime(e.time)}</td>
                      ...
                      <td style={{ padding: '0.5rem', fontFamily: 'monospace' }}>{e.fromKmh.toFixed(0)} → {e.toKmh.toFixed(0)} km/h</td>
                      <td style={{ padding: '0.5rem', fontFamily: 'monospace' }}>{e.distanceM.toFixed(0)} m</td>
                    </tr>
                  ))}
